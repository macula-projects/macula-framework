 ...
 */
package org.macula.boot.core.hibernate.audit;
...
import org.macula.boot.core.utils.BatchNoUtils;
 ...
 */
public class AuditedEventListener implements PostUpdateEventListener, PostDeleteEventListener {
		...
		if (isEntityAudited(event.getEntity())) {
			if (event.getEntity() instanceof Persistable) {
				...
				if (null != id) {
					...
					// 本次变化批号
					String batchNo = BatchNoUtils.gen();
					...
					for (int i = 0; i < propertyNames.length; i++) {
						...
						if (isPropertyAudited(event.getEntity(), propertyName) && isChanged(oldValue, newValue)) {
							...
							if (log.isDebugEnabled()) {
								log.debug("Publish AuditChangedEvent: {}", ToStringBuilder.reflectionToString(chg));
							}
							ApplicationContext.publishApplicationEvent(new AuditChangedEvent(chg));
						}
					}
				}
			}
		...
		if (isEntityAudited(event.getEntity())) {
			if (event.getEntity() instanceof Persistable) {
				...
				if (null != id) {
					...
					ApplicationContext.publishApplicationEvent(new AuditDeletedEvent(chg));
				}
			}
		}
	}
	...
	private boolean isChanged(Object oldValue, Object newValue) {
		if (oldValue instanceof Persistable || newValue instanceof Persistable || oldValue instanceof Collection
				|| newValue instanceof Collection || oldValue == newValue) {
			return false;
		}
