package org.macula.boot.core.cache.cache.redis;
...
import org.macula.boot.core.cache.support.AwaitThreadContainer;
import org.macula.boot.core.cache.support.Lock;
import org.macula.boot.core.cache.support.NullValue;
import org.macula.boot.core.cache.support.ThreadTaskUtils;
...
public class RedisCache extends AbstractValueAdaptingCache {
    ...
    public void put(Object key, Object value) {
        RedisCacheKey redisCacheKey = getRedisCacheKey(key);
        logger.debug("redis缓存 key= {} put缓存，缓存值：{}", redisCacheKey.getKey(), JSONUtils.objectToJson(value));
        putValue(redisCacheKey, value);
    }

    @Override
    public Object putIfAbsent(Object key, Object value) {
        logger.debug("redis缓存 key= {} putIfAbsent缓存，缓存值：{}", getRedisCacheKey(key).getKey(), JSONUtils.objectToJson(value));
    ...
    private <T> T loaderAndPutValue(RedisCacheKey key, Callable<T> valueLoader, boolean isLoad) {
        ...
        try {
            ...
            logger.debug("redis缓存 key={} 执行被缓存的方法，并将其放入缓存, 耗时：{}。数据:{}", key.getKey(), System.currentTimeMillis() - start, JSONUtils.objectToJson(result));
